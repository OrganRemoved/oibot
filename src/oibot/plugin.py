...
from oibot.bot import BotIdentity, OiBot
...
class PluginManager:
    ...
    def __init__(self, bot: OiBot) -> None:
        ...
        self.sessions: dict[
            tuple[BotIdentity, str | None, str | None], asyncio.Future
        ] = {}

    async def __call__(self, ctx: Context) -> Any:
        async with Event(self.bot, ctx) as event:
            if future := self.sessions.get(
                (
                    self.bot.bot_identity.get(),
...
def on(
    ...
    async def resolve_dependency(event: Event, dependency: Dependency) -> Any:
        ...
        if use_cache := dependency.use_cache:
            if future := event.dependency_cache.get(dependency.dependency):
                return await future

            event.dependency_cache[dependency.dependency] = future = (
        ...
        try:
                ...
                for param_name, param in sign.parameters.items():
                    ...
                    if isinstance(param.default, Dependency):
                        tasks[param_name] = tg.create_task(
                            resolve_dependency(event, param.default)
                        )
            ...
            if isclass(f) and issubclass(f, AbstractAsyncContextManager):
                result = await event.stack.enter_async_context(func(**kwargs))

            elif isclass(f) and issubclass(f, AbstractContextManager):
                result = event.stack.enter_context(func(**kwargs))

            elif isasyncgenfunction(f):
                result = await event.stack.enter_async_context(
                    asynccontextmanager(func)(**kwargs)
                )

            elif isgeneratorfunction(f):
                result = event.stack.enter_context(contextmanager(func)(**kwargs))
